...
class AttendeeRegistrationScreenState extends State<AttendeeRegistrationScreen> {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        final response = await http.post(
          ...
          body: jsonEncode({
            'event_id':int.parse(eventIdController.text),
            ...
            'payment_location': selectedLocation,
          }),
        );
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text(
                  "You are registered successfully.\nPay at: $selectedLocation.\nYour ID: ${jsonResponse['attendee_id']}",
                ),
                actions: [
                  ElevatedButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(titleText: "Register Event", true, false).buildAppBar(),
      body: Padding(
